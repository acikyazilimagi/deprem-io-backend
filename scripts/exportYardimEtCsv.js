...
async function main() {
  ...
  await YardimEt.find({})
    .select(
      'yardimTipi adSoyad telefon sehir hedefSehir aciklama fields ip createdAt updatedAt'
    )
    .then((docs) => {
      writeStream.write(
        'yardimTipi,adSoyad,telefon,sehir,hedefSehir,aciklama,fields,createdAt,updatedAt\n'
      );
      docs.forEach((doc) => {
        writeStream.write(
          `"${doc.yardimTipi}","${doc.adSoyad}","${doc.telefon}","${
            doc.sehir
          }","${doc.hedefSehir}","${doc.aciklama}","${JSON.stringify(
            doc.fields
